    ...
    <div v-show="!loading">
        <v-layout row>
            <v-flex xs12>
                <div class="text-xs-left">
                    ...
                    <v-btn v-if='$route.name == "new-client"' color="primary" dark medium icon flat @click="save">
                        <v-icon>
                            done
                        </v-icon>
                    </v-btn>
        ...
        <v-layout row wrap>
                ...
                <v-layout column>
                    <v-form ref="form">
                        <v-layout row>
                            <v-flex xs12>
                                <v-layout column>
                                    <v-text-field label="Name" data-vv-name="name" v-model="client.name" outline :rules="rules.name" />
                                </v-layout>
                            </v-flex>
                        </v-layout>
                        ...
                        <v-layout row>
                            <v-flex xs12>
                                <v-layout column>
                                    <v-checkbox label="Active" data-vv-name="active" type="checkbox" v-model="client.active" outline />
                                </v-layout>
                            </v-flex>
                        </v-layout>
                    </v-form>
            ...
            <v-flex d-flex xs12 sm12 md4 lg5 pa-4>
                <v-layout column>
                    <MediasManager v-if="renderComponents" ref="medias" :label="'Media'" :limit="1" :existingMedia="client.medias.medias ? client.medias.medias.thumb : []"/>
                </v-layout>
            </v-flex>
        </v-layout>
    </div>
